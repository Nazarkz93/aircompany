...
public class AirportTest {
    ...
    public void testNextPlaneMaxLoadCapacityIsHigherThanCurrent() {
        ...
        boolean isSorted = IntStream.range(0, planesSortedByMaxLoadCapacity.size() - 1)
                .allMatch(i -> planesSortedByMaxLoadCapacity.get(i).getMinLoadCapacity() <= planesSortedByMaxLoadCapacity.get(i + 1).getMinLoadCapacity());
    ...
    public void testHasAtLeastOneBomberInMilitaryPlanes() {
        ...
        long bomberCount = bomberMilitaryPlanes.stream()
                .filter(plane -> plane.getType() == MilitaryType.BOMBER)
                .count();
        Assert.assertTrue(bomberCount > 0);
    }
